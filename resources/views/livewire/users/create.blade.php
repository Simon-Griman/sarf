<form wire:submit.prevent="save" onsubmit="document.activeElement.blur()">
    <div class="space-y-12 sm:px-6" style="height: 79vh">
        ...
        <div class="mt-6 flex items-center justify-end gap-x-6 px-6 pb-4">
            <x-cancel-button href="{{ route('users.index') }}" @keydown.window="if ($event.key.toLowerCase() === 'escape' && !$event.target.closest('input, textarea, select')) { window.location.href = '{{ route('users.index') }}' }"/>
            <x-primary-button @keydown.window.ctrl.enter.prevent="document.activeElement.blur(); $nextTick(() => $wire.save())">Guardar</x-primary-button>
        </div>
    </div>
</form>
