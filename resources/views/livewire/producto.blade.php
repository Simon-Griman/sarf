<div>
    <x-card class="text-center pt-0">
        ...
        <x-text-input wire:model.live.debounce.400ms="nombre" placeholder="Buscar:" class="w-full mb-2" />
                ...
                @foreach($productos as $producto)
                    <tr class="hover:bg-gray-100 dark:hover:bg-zinc-700">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <x-edit-button class="text-amber-600 dark:text-amber-500 hover:text-amber-500 dark:hover:text-amber-400" wire:click="modalEditar({{ $producto->id }})"><i class="fas fa-pen"></i> Editar</x-edit-button>
                            <x-delete-button class="ml-2" wire:click="eliminar({{ $producto->id }})"><i class="fas fa-trash"></i> Eliminar</x-delete-button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
                    ...
                    <form @if ($accion == 'crear') wire:submit.prevent="crear" @elseif ($accion == 'editar') wire:submit.prevent="editar" @endif    >
                        ...
                        <div class="bg-white px-4 pt-5 pb-2 sm:px-6 dark:bg-zinc-800 border-b border-zinc-700">
                            <h2 class="text-xl font-semibold text-gray-900 dark:text-white" id="modal-title">
                                @if ($accion == 'crear') Crear Nuevo @else Editar @endif Producto
                            </h2>
                        </div>

                        <div class="overflow-y-auto p-4 sm:p-6 bg-white dark:bg-zinc-800 custom-scrollbar">
                            <div class="mt-2" x-ref="focusInput">
                                <x-input-label for="producto" value="Nombre del producto" />
                                <x-form-text-input wire:model="producto" name="producto" />
                            </div>
                        </div>
